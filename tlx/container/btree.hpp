        ...
        bool slice_insert(int slicenum, idx_t pos, const value_type& data) {
            ...
            if constexpr (concurrent) {
                ...
                TLX_BTREE_ASSERT(nodep->mutex_.write_locked() ||
                                 slices[slicenum].lock.write_locked());
#endif
...
#pragma GCC diagnostic pop
            ...
            std::copy(idx_ar + pos + 1, idx_ar + slices[slicenum].slotuse,
                    idx_ar + pos);
            (*slotusep)--;
            if constexpr (concurrent) {
                if constexpr (optimism) {
...
public:
    ...
    struct LeafNode : public node {
        ...
        const value_type& get_overall(int i, MaplKeyContext *ctx) const {
            TLX_BTREE_ASSERT(mapl);
            TLX_BTREE_ASSERT(i >= ctx->prev_index && i < node::slotuse);

            idx_t slice_i = ctx->prev_slice + 1;
            idx_t idx = i - ctx->prev_index;
...
[[maybe_unused]] retry_no_lock:
            if (!leaf->mapl) {
                ...
                LOG_STR("insert done " << leaf << " slot=" << slot << " min="
                        << leaf->key(0) << " max=" << leaf->key(leaf->slotuse - 1) << "\n");
                ...
                if constexpr (concurrent && optimism) {
                    ...
                    if (leaf->is_full()) {
                        leaf->unmaplize(this);
                        goto retry_no_lock; // let non-mapl code handle split
                    }
                    if (slice->is_full()) {
                        leaf->mapl->rebalance();
                    ...
                    if (slice->is_full()) {
                        ...
                        goto retry;
#else
                        // already has leaf in write lock which is expected
#endif
                    }
                ...
                if (!successful) {
#ifdef MAPL_NO_LOCK
                    TLX_BTREE_ASSERT(false); // leaf write locked and wasn't full, so this can't happen
#endif
    ...
    void split_leaf_node(LeafNode* leaf,
        ...
        LOG_STR("before split leaf=" << leaf << " min=" << leaf->min_key() << " max=" << leaf->max_key());
        ...
        *out_newleaf = newleaf;
        ...
        LOG_STR("after split leaf=" << leaf << " min=" << leaf->min_key() << " max=" << leaf->max_key());
        LOG_STR("after split newleaf=" << leaf << " min=" << newleaf->min_key() << " max=" << newleaf->max_key());
    }
...
private:
    ...
    enum result_flags_t {
        ...
        btree_will_update_lastkey = 8,
        btree_will_soon_underflow = 16,
    ...
    std::pair<result_t, bool> erase_one_descend(const key_type& key,
        ...
        if (curr->is_leafnode())
        {
            ...
            if (!leaf->mapl)
            {
                ...
                if constexpr (concurrent && optimism) {
                    if (slot == leaf->slotuse - 1) { //delete the last key
                        leaf->mutex_.write_unlock();
                        return {btree_will_update_lastkey, true};
                    }
                    if (leaf->soon_underflow() &&
                        !(leaf == root_ && leaf->slotuse >= 2)) {
                        leaf->mutex_.write_unlock();
                        return {btree_will_soon_underflow, true};
                    }
                ...
                Slice& slice = leaf->mapl->slices[slicenum];
                if constexpr (concurrent && optimism) {
                    // in pessimistic mode leaf has the right lock, no need
                    // to lock the slice
                    slice.lock.write_lock();
                }
...
#if 0 //ndef NDEBUG
                {
                    std::stringstream ss;
                    leaf->print_mapl(ss);
                    LOG_STR("before erase leaf " << leaf << " slice:" << slicenum
                            << " ind:" << ind << " " << ss.str());
                }
#endif
...
#if 0 //ndef NDEBUG
                {
                    std::stringstream ss;
                    leaf->print_mapl(ss);
                    LOG_STR("after erase leaf " << leaf << " slice:" << slicenum
                            << " ind:" << ind << " " << ss.str());
                }
#endif
        ...
        else // !curr->is_leafnode() current node is inner node
        {
            ...
            bool try_lock_optimism = optimism;
            auto lock_p = &inner->mutex_;
