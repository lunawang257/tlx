...
class Random {
   ...
   public:
    ...
    Random(IntType lower, IntType upper)
        : device{}, engine{device()}, dist{lower, upper} {}
...
class Timer {
   ...
   public:
    ...
    /*
     * Stop() - Stops timer and returns the duration between the previous
     * Start() and the current Stop()
     *
    ...
    /*
     * GetInterval() - Returns the length of the time interval between the
...
class Envp {
   public:
     ...
     *
     * If the env var is found and the value is parsed correctly then return
     * true If the env var is not found then retrun true, and value_p is not
     * modified If the env var is found but value could not be parsed correctly
     * then return false and value is not modified
     */
...
class Zipfian {
   private:
     ...
     *
     * This function is adapted from the above link, which was again adapted
     * from
