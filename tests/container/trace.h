...
public:
    ...
    uint32_t Random () {
        ...
        // sign-bit-based test.
        if (static_cast<uint32_t>(seed_) > M) {
            seed_ -= M;
        }
        return seed_;
    }
    ...
    uint64_t Random64 () {
        // 62 bit random value;
        const uint64_t rand64 = ((static_cast<uint64_t> (random ())) << 31) + (static_cast<uint64_t> (random ()));
        return rand64;
    }

    double RandomDouble () {
        // random between 0.0 - 1.0
        const double r = static_cast<double> (Random64 ());
...
private:
    ...
    double* zetalist_double = reinterpret_cast<double*> (zetalist_u64);
...
TraceZipfian::TraceZipfian (int seed, uint64_t minimum, uint64_t maximum, double zipfian_constant)
    ...
    gz->zetan = zetan;
    gz->eta = (1.0 - std::pow (2.0 / (static_cast<double> (items)), 1.0 - zipfian_constant)) /
...
double TraceZipfian::ZetaRange (const uint64_t start, const uint64_t count, const double theta) {
    ...
    for (uint64_t i = 0; i < count; i++) {
        sum += (1.0 / pow (static_cast<double>(start + i + 1), theta));
    }
    return sum;
}
...
uint64_t TraceZipfian::NextRaw () {
    ...
    const double x = (static_cast<double> (gz->nr_items)) * pow (gz->eta * (u - 1.0) + 1.0, gz->alpha);
    const uint64_t ret = gz->base + static_cast<uint64_t> (x);
    return ret;
}
